...
statement:		command SEP					{ prompt(); }
		...
		|	error '\n' 					{ yyerrok; prompt(); }
		;
command:	GOTO expression expression	{ goto_func($2, $3); }
		|	WHERE						{ where_func(); }
		|	PENUP						{ penup(); }
		|	PENDOWN						{ pendown(); }
		...
		|	SAVE STRING 			{ save($2); }
		;
expression:		NUMBER PLUS expression				{ printf("%.2f\n", $$ = $1 + $3); }
		|	NUMBER MULT expression				{ printf("%.2f\n", $$ = $1 * $3); }
		|	NUMBER SUB expression				{ printf("%.2f\n", $$ = $1 - $3); }
		|	NUMBER DIV expression				{ printf("%.2f\n", $$ = $1 / $3); }
		|	NUMBER								{ $$ = $1; }
		;
